 ...
 *   Usaba `if (passwordEncoder.matches(...))` en lugar de `if (!passwordEncoder.matches(...))`.
 *   Consecuencia: lanzaba excepción cuando la contraseña era CORRECTA (autenticación rota).
 *   Bug corregido: línea 116 de UserServiceImpl ahora usa `!passwordEncoder.matches(...)`.
 */
